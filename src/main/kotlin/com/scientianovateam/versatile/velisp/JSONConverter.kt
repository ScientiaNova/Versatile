...
fun convertJSON(json: JsonElement): IUnresolved = when(json) {
    is JsonArray -> FunctionCall((((json.firstOrNull() ?: throw IllegalStateException("Empty JSON array")) as? JsonPrimitive)?.asString ?: throw IllegalStateException("Invalid start of a function call")), json.drop(1).map(::convertJSON))
