...
public class SimpleMachineRecipe {
	...
	public List<List<ItemStack>> getInputsAsList() {
		return Arrays.stream(input).map(o -> {
			...
			return stack.getTag().getAllElements().stream().map(i -> new ItemStack(i, stack.getCount())).collect(Collectors.toList());
	...
	public boolean isInputValid(ItemStack[] stacks) {
		...
		return stacks.length == Arrays.stream(input).map(stackRec -> Arrays.stream(stacks)
				.filter(stack -> stackRec instanceof ItemStack ? (stack.isItemEqual((ItemStack) stackRec) && stack.getCount() >= ((ItemStack) stackRec).getCount()) : ((TagStack) stackRec).getTag().contains(stack.getItem()) && stack.getCount() >= ((TagStack) stackRec).getCount())
	...
	public int getIndexOfInput(ItemStack stack) {
		Object o = Arrays.stream(input).filter(input -> input instanceof ItemStack ? stack.isItemEqual((ItemStack) input) : ((TagStack) input).getTag().contains(stack.getItem())).findFirst().get();
		if (o == null)
			return -1;
	...
	public boolean itemBelongsInRecipe(ItemStack stack) {
		...
		return Arrays.stream(input).anyMatch(stackRec -> {
			if (stackRec instanceof ItemStack) {
				...
				return ((TagStack) stackRec).getTag().contains(stack.getItem());
		});
	}
