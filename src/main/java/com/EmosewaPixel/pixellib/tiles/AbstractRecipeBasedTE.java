...
public abstract class AbstractRecipeBasedTE<T extends SimpleMachineRecipe> extends ProgressiveTE {
	...
	public AbstractRecipeBasedTE(TileEntityType type, AbstractRecipeList<T, ?> recipeList) {
		...
		recipeInventory = new ImprovedItemStackHandler(recipeList.getMaxRecipeSlots(), StreamUtils.INSTANCE.getArrayFromRange(0, recipeList.getMaxInputs()), StreamUtils.INSTANCE.getArrayFromRange(recipeList.getMaxInputs(), recipeList.getMaxRecipeSlots())) {
			@Override
	...
	protected void work() {
		...
		StreamUtils.INSTANCE.repeat(recipeList.getMaxInputs(), i -> {
	...
	public void dropInventory() {
		StreamUtils.INSTANCE.repeat(combinedHandler.getSlots(), i -> ItemUtils.INSTANCE.spawnItemInWorld(world, pos, combinedHandler.getStackInSlot(i)));
	}
