...
open class SimpleMachineRecipe(val inputs: Array<Pair<IRecipeStack<ItemStack>, Float>>, val fluidInputs: Array<Pair<IRecipeStack<FluidStack>, Float>>, val outputs: Array<WeightedMap<out IRecipeStack<ItemStack>>>, val fluidOutputs: Array<WeightedMap<out IRecipeStack<FluidStack>>>, val time: Int) {
    val outputStackLists get() = outputs.flatMap { it.values.map(IRecipeStack<ItemStack>::stacks) }

    val fluidOutputStackLists get() = fluidOutputs.flatMap { it.values.map(IRecipeStack<FluidStack>::stacks) }

    val inputStackLists get() = inputs.map { it.first.stacks }

    val outputStacks get() = outputStackLists.map(List<ItemStack>::first)

    val fluidInputStackLists get() = fluidInputs.map { it.first.stacks }

    val fluidOutputStacks get() = fluidOutputStackLists.map(List<FluidStack>::first)

    val inputItemRecipeStacks get() = inputs.map(Pair<IRecipeStack<ItemStack>, Float>::first)

    val inputFluidRecipeStacks get() = fluidInputs.map(Pair<IRecipeStack<FluidStack>, Float>::first)

    val inputRecipeStacks: List<IRecipeStack<*>> get() = inputItemRecipeStacks + inputFluidRecipeStacks
