    ...
    companion object {
        ...
        fun processPacket(packet: UpdateFluidStackPacket, context: Supplier<NetworkEvent.Context>) {
            context.get().enqueueWork {
                ...
                (te?.properties?.get(packet.property) as? IFluidHandlerModifiable)?.setFluidInTank(packet.slot, packet.stack)
                ((context.get().sender?.openContainer as? BaseContainer)?.clientProperties?.get(packet.property) as? IFluidHandlerModifiable)?.setFluidInTank(packet.slot, packet.stack)
            }
        }
    }
}