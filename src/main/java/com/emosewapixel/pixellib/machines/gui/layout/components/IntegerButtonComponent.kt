...
open class IntegerButtonComponent(override val property: IVariableProperty<Int>, val textures: List<GUITexture>, override val x: Int, override val y: Int) : IPropertyGUIComponent {
    ...
    override fun drawInBackground(mouseX: Int, mouseY: Int, screen: BaseScreen) {
        textures.getOrNull(property.value)?.render(screen.guiLeft + x, screen.guiTop + y, width, height)
    }
    ...
    override fun onMouseClicked(mouseX: Double, mouseY: Double, clickType: Int, screen: BaseScreen): Boolean {
        ...
        property.setValue(if (value + 1 < textures.size) value + 1 else 0)
        return true
    }
}