...
import com.emosewapixel.pixellib.capabilities.FluidStackHandler
...
abstract class AbstractRecipeBasedTE<T : SimpleMachineRecipe>(type: TileEntityType<*>, val recipeList: AbstractRecipeList<T, *>) : ProgressiveTE(type) {
    ...
    protected open val fluidCapacityPerTank = 4000

    protected var recipeInventory = object : ImprovedItemStackHandler(recipeList.maxRecipeSlots, 0 until recipeList.maxInputs, recipeList.maxInputs until recipeList.maxRecipeSlots) {
    ...
    protected var recipeTanks = FluidStackHandler((0 until recipeList.maxRecipeTanks).map { FluidTank(fluidCapacityPerTank) }, 0 until recipeList.maxFluidInputs, 0 until recipeList.maxRecipeTanks)
    ...
    override fun <T> getCapability(cap: Capability<T>, side: Direction?): LazyOptional<T> = when {
        cap === CapabilityItemHandler.ITEM_HANDLER_CAPABILITY -> LazyOptional.of { recipeInventory }.cast()
        cap === CapabilityFluidHandler.FLUID_HANDLER_CAPABILITY -> LazyOptional.of { recipeTanks }.cast()
        else -> LazyOptional.empty()
    }

    protected fun canOutput(recipe: SimpleMachineRecipe) = recipe.outputStacks.indices.all { recipeInventory.insertItem(recipeList.maxInputs + it, recipe.outputStacks[it].copy(), true).isEmpty }
    ...
    protected fun output(recipe: SimpleMachineRecipe) {
        ...
        recipe.outputStacks.indices.forEach { i ->
            if (recipe.getOutputChance(i) >= rand.nextFloat())
                recipeInventory.insertItem(recipeList.maxInputs + i, recipe.outputStacks[i].copy(), true)
        }
    }
